...
let x_spacecraftName: string = 'Determination';
let x_speedMph:	number	= 17500;
let kilometersToMars:	number	= 225000000;
let kilometersToTheMoon:	number	= 384400;
let x_milesPerKilometer:	number	= 0.621;


// Part 2: Print Days to Mars
let milesToMars: number = kilometersToMars * x_milesPerKilometer;
let hoursToMars: number = milesToMars / x_speedMph;
...
function getDaysToLocation(kilometersAway:number):number {
    let milesAway: number = kilometersAway * x_milesPerKilometer;
    let hoursToLocation: number = milesAway / x_speedMph;
